...
var favoritelist = require("./FavoriteSchema");
    ...
    var AllQueries = function() {
        ...
        var getRequest = function(db, successCallback, errorCallback, filter) {
            ...
            db.once('open', function() {
                favoritelist.find(filter, function (err, list) {
                    var response = {};
                    if (err) {
                        errorCallback( handleUnwantedRequest("Data Error", err) );
                    }
                    else {
                        successCallback({"Favorites": list});
                    }
                    mongoose.disconnect();
                });
            });
        ...
        var postRequest = function(db, successCallback, errorCallback, data) {
            db.once('open', function() {
                var list = new favoritelist(data);
                favoritelist.find({name: data.name}, function (err, em) {
                    if ( em.length === 0 ) {
                        list.save(function (err, emp) {
                            var response = {};
                            if (err) {
                                errorCallback( handleUnwantedRequest("Data Error", err) );
                            }
                            else {
                                successCallback({"Message": "Successfully Added", "List": emp});
                            }
                            mongoose.disconnect();
                        });
                    }
                    else {
                        errorCallback( handleUnwantedRequest("Duplicate Entry", "List Name " + data.name + " already registered") );
                    }
                });              
            });
        };
        ...
        var patchRequest = function(db, successCallback, errorCallback, data) {
            db.once('open', function() {
                favoritelist.findOneAndUpdate(data.query, data.change, function (err, emp) {
                    var response = {};
                    if (err) {
                        errorCallback( handleUnwantedRequest("Data Error", err) );
                    }
                    else {
                        successCallback({"Message": "Successfully Updated", "Favorite": data.query});
                    }
                    mongoose.disconnect();
                });
            });
        ...
        var deleteRequest = function(db, successCallback, errorCallback, data) {
            db.once('open', function() {
                ...
                favoritelist.find( data, function (err1, em) {
                    if ( em.length !== 0 ) {
                        favoritelist.remove(data, function (err, emp) {
                            var response = {};
                            if (err) {
                                errorCallback( handleUnwantedRequest("Data Error", err) );
                            }
                            else {
                                successCallback({"Message": "Successfully Deleted", "List": data});
                            }
                            mongoose.disconnect();
                        });     
                    }
                    else {
                        errorCallback( handleUnwantedRequest("Invalied List", "List " + data.name + " does not exist") );
                    }
                });
            });
        };
